...
class TemplateDeployment(unittest.TestCase):
 def test_templates_release_preserves_configuration_and_copies_one_file(self):
  image='gcr.io/usta-prod/usta-loops@sha256:'+'a'*64
  before=b'synthetic app';template=b'synthetic template';seen={}
  ...
  def run(cmd,**kw):
   if cmd[:2]==['docker','cp']:
    Path(cmd[-1]).write_bytes(template if 'templates.py' in cmd[-2] else before)
  ...
  with patch.object(d,'gjson',side_effect=gj),patch.object(d.subprocess,'check_output',side_effect=output),patch.object(d.subprocess,'run',side_effect=run),patch.object(sys,'argv',['deploy','--templates-only','--expected-image',image,'--expected-app-sha256',hashlib.sha256(before).hexdigest(),'--expected-templates-sha256',hashlib.sha256(template).hexdigest()]):d.main()
  self.assertEqual(seen['env'],[{'name':'CONFIG','value':'fixture'}])
  self.assertEqual(seen['files'],['Dockerfile','loops/service/apps/templates.py'])
 def test_missing_template_proof_refuses_before_provider_call(self):
  with patch.object(d,'gjson',side_effect=AssertionError('provider reached')),patch.object(sys,'argv',['deploy','--templates-only','--expected-app-sha256','a'*64]):
