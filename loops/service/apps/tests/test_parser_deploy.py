...
class Deployment(unittest.TestCase):
 def test_matching_release_preserves_configuration_and_copies_one_file(self):
  image='gcr.io/usta-prod/usta-loops@sha256:'+'a'*64
  before=b'synthetic app';parser=b'synthetic parser';seen={}
  ...
  def run(cmd,**kw):
   if cmd[:2]==['docker','cp']:
    Path(cmd[-1]).write_bytes(parser if 'matching.py' in cmd[-2] else before)
  ...
  with patch.object(d,'gjson',side_effect=gj),patch.object(d.subprocess,'check_output',side_effect=output),patch.object(d.subprocess,'run',side_effect=run),patch.object(sys,'argv',['deploy','--matching-only','--expected-image',image,'--expected-app-sha256',hashlib.sha256(before).hexdigest(),'--expected-matching-sha256',hashlib.sha256(parser).hexdigest()]):d.main()
  self.assertEqual(seen['env'],[{'name':'CONFIG','value':'fixture'}])
  self.assertEqual(seen['files'],['Dockerfile','loops/service/apps/matching.py'])
 def test_missing_parser_proof_refuses_before_provider_call(self):
  with patch.object(d,'gjson',side_effect=AssertionError('provider reached')),patch.object(sys,'argv',['deploy','--matching-only','--expected-app-sha256','a'*64]):
