...
Uses the existing local model route only; otherwise saves the prompt for a person.
Remote/CLI routes are inactive. Invocation outcome never establishes billed cost.
...
def ask_astra(prompt: str) -> None:
    """Historical compatibility entry point; this job has no authorized CLI route.

    CLI subscription admission is separate from money authority. Never infer a
    debit from a model invocation or activate a new route from recorded revenue.
    """
    return None
...
def main() -> int:
    ...
    if answer is None:
        ...
        used = LOCAL_MODEL if answer else used
    ...
    if answer is None:
        ...
        return 1
    out.write_text(f"# loops weekly {today}\n\nConfigured model: {used}. Observed model: UNKNOWN. Call outcome: response received. Billed cost: UNKNOWN. This is a proposal; nothing was changed.\n\n"
                   f"{answer}\n", encoding="utf-8")
