...
def write(spec: dict) -> Path:
    ...
    if dest.is_file():
        ...
        refuse_hand_edit(str(spec["id"]), dest)
    dest.parent.mkdir(parents=True, exist_ok=True)
    out = generated_marker(str(spec["id"])) + render(spec)
    dest.write_text(out, encoding="utf-8")
    record_generated(str(spec["id"]), out)
    return dest


# ---------------------------------------------------------------------------
# Hand-edit guard (operator 2026-09-15: "all fixes should be permanent").
#
# On 2026-09-15 twenty-two approved copy repairs were typed straight into
# families/<id>/index.html of generated families, committed, and then thrown
# away by the next build, which rewrote every one of them from family_spec().
# Two things stop that from happening again:
#   1. every generated page opens with a comment naming the file that owns it;
#   2. the builder remembers a fingerprint of what it last wrote, and refuses to
#      overwrite a page whose bytes no longer match -- someone edited it by hand
#      and that edit belongs in the spec, not in the bin.
# Set SLICE_ACCEPT_OVERWRITE to "all" or a comma-separated list of family ids
# to knowingly discard a hand edit. Nothing else bypasses the refusal.
# ---------------------------------------------------------------------------
GENERATED_HASH_DIR = ROOT / "var" / "generated-hash"


def generated_marker(fid: str) -> str:
    mod = "scripts/slice_" + fid.replace("-", "_") + ".py"
    return (
        "<!-- GENERATED FILE. Do not edit this page by hand: the build rewrites it "
        f"from family_spec() in {mod}. Change the spec there, then run "
        f"python3 scripts/build_slices.py --only {fid} -->\n"
    )
...
def record_generated(fid: str, out: str) -> None:
    GENERATED_HASH_DIR.mkdir(parents=True, exist_ok=True)
    (GENERATED_HASH_DIR / f"{fid}.sha256").write_text(_sha(out) + "\n", encoding="utf-8")


def refuse_hand_edit(fid: str, dest: Path) -> None:
    import os

    rec = GENERATED_HASH_DIR / f"{fid}.sha256"
    if not rec.is_file():
        return  # never generated by this guard yet; nothing to compare against
    accept = {x.strip() for x in os.environ.get("SLICE_ACCEPT_OVERWRITE", "").split(",") if x.strip()}
    if "all" in accept or fid in accept:
        return
    on_disk = _sha(dest.read_text(encoding="utf-8"))
    if on_disk != rec.read_text(encoding="utf-8").strip():
        raise ValueError(
            f"{fid}: {dest.relative_to(ROOT)} was edited by hand after the last build "
            f"(its bytes no longer match what the generator wrote). Building now would "
            f"throw that edit away. Port the change into family_spec() in "
            f"scripts/slice_{fid.replace('-', '_')}.py, or set SLICE_ACCEPT_OVERWRITE={fid} "
            f"to discard it on purpose. Nothing was written."
        )
