#!/usr/bin/env python3
"""Offline selftest for ledgermatch's fv5 delivery module. Exits 0 when
...
def main() -> int:
    ...
    check(len(cf) == 1, f"{len(cf)} custom fields; ledgermatch must declare exactly 1")
    ...
    if page is not None:
        ...
        check("lp1.ledgermatch." in page, "page carries a ledgermatch pro key")
        ...
        check("It never posts entries or moves money" in page, "page states the ledgermatch honest line")
    ...
    if bad_page is not None:
        ...
        check("lp1.ledgermatch." in bad_page, "the key still delivers even when the domain is unreadable")
    ...
    if FAILS:
        ...
        return 1
    print(f"SELFTEST ok: ledgermatch, {len(FAILS)} failures")
    return 0
