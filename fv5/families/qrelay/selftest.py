#!/usr/bin/env python3
"""Offline selftest for qrelay's fv5 delivery module. Exits 0 when
...
def main() -> int:
    ...
    check(len(cf) == 1, f"{len(cf)} custom fields; qrelay must declare exactly 1")
    ...
    if page is not None:
        ...
        check("lp1.qrelay." in page, "page carries a qrelay pro key")
        ...
        check("We never say a vendor is secure" in page, "page states the qrelay honest line")
    ...
    if bad_page is not None:
        ...
        check("lp1.qrelay." in bad_page, "the key still delivers even when the domain is unreadable")
    ...
    if FAILS:
        ...
        return 1
    print(f"SELFTEST ok: qrelay, {len(FAILS)} failures")
    return 0
